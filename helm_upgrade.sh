...
DEFAULT_AUTO_UPDATED_VALUES_FILE=`[ -f values.auto-updated.yaml ] && echo "-f values.auto-updated.yaml"`
...
if [ `./read_yaml.py "environments/${K8S_ENVIRONMENT_NAME}/values.yaml" global enableRootChart` == "true" ]; then
    helm upgrade $DEFAULT_VALUES_FILE $DEFAULT_AUTO_UPDATED_VALUES_FILE $ENVIRONMENT_VALUES_FILE $AUTO_UPDATED_VALUES_FILE "${K8S_HELM_RELEASE_NAME}-${K8S_ENVIRONMENT_NAME}" . "$@" \
