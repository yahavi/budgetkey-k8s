...
```
./run_docker_ops.sh ENVIRONMENT_NAME "kubectl get pods" "orihoch/sk8sops" OriHoch/budgetkey-k8s master secret-k8s-ops.json
```
...
This command should run from the root of the external app, assuming the budgetkey-k8s repo is a sibling directory:

```
travis encrypt-file ../budgetkey-k8s/secret-budgetkey-k8s-ops.json secret-budgetkey-k8s-ops.json.enc
```
...
```
gcloud compute addresses create budgetkey-ENVIRONMENT_NAME-traefik --region=us-central1
```

Get the static IP address:

```
gcloud compute addresses describe budgetkey-ENVIRONMENT_NAME-traefik --region=us-central1 | grep ^address:
```
...
```
git clone git@github.com:OriHoch/budgetkey-k8s.git
```
...
```
gcloud compute ssh budgetkey-k8s-persistent-storage-vm
```
...
```
gcloud auth login
git clone https://github.com/OriHoch/budgetkey-k8s.git ~/budgetkey-k8s
cd ~/budgetkey-k8s
source switch_environment.sh <ENVIRONMENT_NAME>
